...
if [[ "$num_files" -eq 0 ]]; then
    ...
    "${alignment_tool}" -x "$bname" -U "$fq_r1" -S "${out_prefix}_aligned.sam" -p "$THREADS" --very-sensitive # -k 10
else
    ...
    "${alignment_tool}" -x "$bname" -1 "$fq_r1" -2 "$fq_r2" -S "${sam_prefix}_aligned.sam" -p "$THREADS" --very-sensitive # -k 10
fi
